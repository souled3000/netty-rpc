...
public class InvitationCfmApi extends HandlerAdapter {
	...
	public Object rpc(RpcRequest req) throws Exception {
		...
		try {
			...
			String mnick = j.hget("user:nick", operFamily);
			...
			if ("yes".equals(asw)) {
				...
				j.publish("PubCommonMsg:0x36".getBytes(), Utils.genMsg(memlist, 1/** 用户同意加入家庭 */ , Long.parseLong(uId), msg.toString()));
			} else {
				j.publish("PubCommonMsg:0x36".getBytes(), Utils.genMsg(String.valueOf(oper) + "|", 6/** 用户拒绝加入家庭 */ , Long.parseLong(uId), msg.toString()));
